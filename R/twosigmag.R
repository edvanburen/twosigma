...
##' @param index_test List of indices corresponding to rows of the count matrix that are in the test set.
...
##' @param covar_to_test Covariate used for reporting direction (as Up or Down) of the test set. Either a string indicating the name of the covariate to use or an integer giving its associated position in the mean_covar argument.
...
##' @param id Vector of individual-level (sample-level) ID's. Used to estimate inter-gene correlation and for random effect prediction (if present).
...
twosigmag<-function(count_matrix,index_test,index_ref=NULL,all_as_ref=FALSE,covar_to_test,mean_covar,zi_covar
  ...
  required_args<-c("count_matrix","index_test","covar_to_test","mean_covar","zi_covar","id")
  ...
  for(i in 1:ngenes){
    l<-genes[i]
      fit_twosigmag<-lr.twosigma(count_matrix[l,,drop=FALSE],covar_to_test = covar_to_test
      ...
      if(is.numeric(covar_to_test)){
        # +1 for the intercept
        avg_logFC_gene_level[l]<-sum_fit_alt[covar_to_test+1,'Estimate']
      }else{
        avg_logFC_gene_level[l]<-sum_fit_alt[grepl(covar_to_test,names),'Estimate']
      }
  ...
  names(direction)<-names(index_test)
   if(return_summary_fits==TRUE){
     return(list(summary_gene_level_fits=fit,LR_stats_gene_level_all=stats_all,p.vals_gene_level=p.vals_gene_level,set_p.val=p.val,set_p.val_ttest=p.val_ttest,avg_logFC_gene_level=avg_logFC_gene_level,direction=direction,corr=rho_est,test_sets=index_test,ref_sets=index_ref))
   }else{
    return(list(LR_stats_gene_level_all=stats_all,p.vals_gene_level=p.vals_gene_level,set_p.val=p.val,set_p.val_ttest=p.val_ttest,avg_logFC_gene_level=avg_logFC_gene_level,direction=direction,corr=rho_est,test_sets=index_test,ref_sets=index_ref,set_p.val_ttest=p.val_ttest))
  }
}
