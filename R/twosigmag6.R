...
twosigmag6<-function(count_matrix,index_test,index_ref=NULL,all_as_ref=FALSE,mean_form,zi_form,mean_form_null=NULL,zi_form_null=NULL
  ...
  comb<-function(residuals_all,stats_all,p.vals_gene_level,
    estimates_gene_level,se_gene_level,...){
    for(r in list(...)){
      residuals_all[r$l,]<-r$residuals_all
      # stats_all[r$l,]<-r$stats_all
      # p.val_gene_level[r$l,]<-r$p.vals_gene_level
      # estimates_gene_level[r$l,]<-r$estimates_gene_level
      # se_gene_level[r$l,]<-r$se_gene_level
      #logLik[r$l]<-r$logLik
      #gene_err[r$l]<-r$gene_err
    }
     #list(stats_all=stats_all,p.vals_gene_level=p.vals_gene_level,se_gene_level=se_gene_level,
      # estimates_gene_level=estimates_gene_level,residuals_all=residuals_all)
    residuals_all
  }
  num_err<-0
  browser()
  a<-foreach(i=1:ngenes,.combine='comb',.multicombine = TRUE,.init=matrix(NA,nrow=ngenes_total,ncol=ncells),.maxcombine = 100)%dopar%{
    #a<-foreach(i=1:ngenes,.maxcombine = 100)%dopar%{
    l<-genes[i]
    ...
    return(list(stats_all=stats_all,p.vals_gene_level=p.vals_gene_level,se_gene_level=se_gene_level,
      estimates_gene_level=estimates_gene_level,residuals_all=residuals_all,logLik=logLik,gene_err=gene_err,l=l))
    #}
