...
twosigmag7<-function(count_matrix,index_test,index_ref=NULL,all_as_ref=FALSE,mean_form,zi_form,mean_form_null=NULL,zi_form_null=NULL
 ...
 #     id=id,cl=cl)
 # a<-parRapply(count_matrix[genes,],FUN=fit_tsg,statistic=statistic,
 #   covar_to_test=covar_to_test,factor_name=factor_name,contrast_matrix=contrast_matrix,
 #   id=id,cl=cl)

  #a<-pblapply(X=genes,FUN=fit_tsg2,statistic=statistic,
  #       covar_to_test=covar_to_test,factor_name=factor_name,contrast_matrix=contrast_matrix,
  #       id=id,cl=cl)

    # b<-pblapply(chunks,FUN=fit_tsg3,statistic=statistic,
    #        covar_to_test=covar_to_test,factor_name=factor_name,contrast_matrix=contrast_matrix,
    #        id=id,ncomps=ncomps,cl=NULL)

   a<-pbmclapply(chunks,FUN=fit_tsg3,statistic=statistic,
               covar_to_test=covar_to_test,factor_name=factor_name,contrast_matrix=contrast_matrix,
               id=id,ncomps=ncomps,mc.cores=ncores)
   #browser()
  ...
  rm(a)
gc()
  #stats_test<-vector('list',length=1)
  #stats_ref<-vector('list',length=1)
